...
(function () {
  ...
  function renderBusiness() {
    ...
    el.innerHTML =
      ...
      field('Logo image URL', 'business.logoUrl', getPath(draft, 'business.logoUrl'), 'Leave blank to keep the colored dot mark');
    wireFields(el);
  }
  ...
  function renderGallery() {
    ...
    el.innerHTML = '<h2>Before / after gallery</h2><p class="section-sub">Leave photo URLs blank to keep clean placeholder frames.</p>' +
      arrayEditorHtml('gallery', 'Photo pair', draft.gallery, [
        { key: 'label', label: 'Caption', type: 'text' },
        { key: 'before', label: 'Before photo URL', type: 'text' },
        { key: 'after', label: 'After photo URL', type: 'text' },
      ]);
  ...
  function publishToGithub() {
    ...
    var apiBase = 'https://api.github.com/repos/' + gh.owner + '/' + gh.repo + '/contents/config.js';

    fetch(apiBase + '?ref=' + encodeURIComponent(gh.branch), {
      ...
      return fetch(apiBase, {
        ...
        body: JSON.stringify({
          message: 'Update site content via Site Editor',
